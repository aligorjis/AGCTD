spearmanccf <-function(x,y,lgm=NULL)
{
  ...
  for ( lg in lgm:0 ){ 
    s1<- y[1:(length(y)-lg)]
    s2<- x[(lg+1):length(x)]
    sccf= (cov(s2,s1,method = "spearman")/sqrt(cov(x ,x ,method = "spearman")*cov(y ,y ,method = "spearman")))
    corr[o+lg]<- sccf
    lagg[o+lg]<- lg
    sccfm<- data.frame(corr,lagg)
  }
  for ( lg in lgm:0 ){ 
    s1<- x[1:(length(x)-lg)]
    s2<- y[(lg+1):length(y)]
...
for (i in 1:8){ 
  for ( j in 1:8){
    for ( k in 1:5){
      if (cc[j]>cc[i]){
        c=spearmanccf(listgene[[i]] , listgene[[j]], lgm = 2 )
        TM[i,j,k]=c$corr[k]}}}
}
A<-TM[,,3:5]
tr<-mean(abs(A))
